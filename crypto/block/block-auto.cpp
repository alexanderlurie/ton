...
bool BurningConfig::skip(vm::CellSlice& cs) const {
  int fee_burn_nom, fee_burn_denom;
  return cs.advance(8)
      && t_Maybe_bits256.skip(cs)
      && cs.fetch_uint_to(32, fee_burn_nom)
      && cs.fetch_uint_to(32, fee_burn_denom)
      && fee_burn_nom <= fee_burn_denom
      && 1 <= fee_burn_denom;
}
...
bool BurningConfig::unpack(vm::CellSlice& cs, BurningConfig::Record& data) const {
  return cs.fetch_ulong(8) == 1
      ...
      && cs.fetch_uint_to(32, data.fee_burn_nom)
      && cs.fetch_uint_to(32, data.fee_burn_denom)
      && data.fee_burn_nom <= data.fee_burn_denom
      && 1 <= data.fee_burn_denom;
}

bool BurningConfig::unpack_burning_config(vm::CellSlice& cs, Ref<CellSlice>& blackhole_addr, int& fee_burn_nom, int& fee_burn_denom) const {
  return cs.fetch_ulong(8) == 1
...
bool BurningConfig::cell_unpack_burning_config(Ref<vm::Cell> cell_ref, Ref<CellSlice>& blackhole_addr, int& fee_burn_nom, int& fee_burn_denom) const {
  if (cell_ref.is_null()) { return false; }
  auto cs = load_cell_slice(std::move(cell_ref));
  return unpack_burning_config(cs, blackhole_addr, fee_burn_nom, fee_burn_denom) && cs.empty_ext();
}
...
bool BurningConfig::pack(vm::CellBuilder& cb, const BurningConfig::Record& data) const {
  return cb.store_long_bool(1, 8)
      ...
      && cb.store_ulong_rchk_bool(data.fee_burn_nom, 32)
      && cb.store_ulong_rchk_bool(data.fee_burn_denom, 32)
      && data.fee_burn_nom <= data.fee_burn_denom
      && 1 <= data.fee_burn_denom;
}

bool BurningConfig::pack_burning_config(vm::CellBuilder& cb, Ref<CellSlice> blackhole_addr, int fee_burn_nom, int fee_burn_denom) const {
  return cb.store_long_bool(1, 8)
      && t_Maybe_bits256.store_from(cb, blackhole_addr)
      && cb.store_ulong_rchk_bool(fee_burn_nom, 32)
...
bool BurningConfig::cell_pack_burning_config(Ref<vm::Cell>& cell_ref, Ref<CellSlice> blackhole_addr, int fee_burn_nom, int fee_burn_denom) const {
  vm::CellBuilder cb;
  return pack_burning_config(cb, std::move(blackhole_addr), fee_burn_nom, fee_burn_denom) && std::move(cb).finalize_to(cell_ref);
}
...
bool BurningConfig::print_skip(PrettyPrinter& pp, vm::CellSlice& cs) const {
  ...
  return cs.fetch_ulong(8) == 1
      ...
      && cs.fetch_uint_to(32, fee_burn_nom)
      && pp.field_int(fee_burn_nom, "fee_burn_nom")
      && cs.fetch_uint_to(32, fee_burn_denom)
