...
engine_validator_overlayStats::engine_validator_overlayStats(td::Bits256 const &overlay_id_, object_ptr<PublicKey> &&overlay_id_full_, td::Bits256 const &adnl_id_, std::vector<object_ptr<adnl_id_short>> &&nodes_, std::vector<object_ptr<engine_validator_oneStat>> &&stats_)
...
#define FAIL(error) p.set_error(error)
  ...
  , nodes_(TlFetchVector<TlFetchObject<adnl_id_short>>::parse(p))
...
void engine_validator_overlayStats::store(td::TlStorerToString &s, const char *field_name) const {
  if (!LOG_IS_STRIPPED(ERROR)) {
    ...
    { const std::vector<object_ptr<adnl_id_short>> &v = nodes_; const std::uint32_t multiplicity = static_cast<std::uint32_t>(v.size()); const auto vector_name = "vector[" + td::to_string(multiplicity)+ "]"; s.store_class_begin("nodes", vector_name.c_str()); for (std::uint32_t i = 0; i < multiplicity; i++) { if (v[i] == nullptr) { s.store_field("", "null"); } else { v[i]->store(s, ""); } } s.store_class_end(); }
