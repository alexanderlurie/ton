...
class engine_validator_overlayStats final : public Object {
 public:
  ...
  td::Bits256 adnl_id_;
  std::vector<object_ptr<adnl_id_short>> nodes_;
  std::vector<object_ptr<engine_validator_oneStat>> stats_;

  engine_validator_overlayStats();

  engine_validator_overlayStats(td::Bits256 const &overlay_id_, object_ptr<PublicKey> &&overlay_id_full_, td::Bits256 const &adnl_id_, std::vector<object_ptr<adnl_id_short>> &&nodes_, std::vector<object_ptr<engine_validator_oneStat>> &&stats_);

  static const std::int32_t ID = -1407190425;
  std::int32_t get_id() const final {
    return ID;
  }
